...
const empty = { nama: '', deskripsi: '', foto_url: '', aktif: true, urutan: 0 }
...
export default function KelolaFasilitas() {
  ...
  async function load() {
    const { data } = await supabase.from('fasilitas').select('*').order('urutan')
    setItems(data || [])
  }
  ...
  async function handleUpload(e) {
    ...
    const path = `fasilitas/${Date.now()}-${compressed.name}`
  ...
  async function handleSubmit(e) {
    e.preventDefault()
    if (editingId) {
      await supabase.from('fasilitas').update(form).eq('id', editingId)
    } else {
      await supabase.from('fasilitas').insert(form)
    }
    setForm(empty)
    setEditingId(null)
    load()
  ...
  async function toggleAktif(item) {
    await supabase.from('fasilitas').update({ aktif: !item.aktif }).eq('id', item.id)
    load()
  }
  ...
  return (
    <DashboardLayout links={adminLinks} title="Admin">
      <h1 className="font-display text-2xl font-bold mb-6">Kelola Fasilitas</h1>

      <form onSubmit={handleSubmit} className="bg-white border border-ink/10 rounded-lg p-5 mb-8 space-y-3">
        <input
          placeholder="Nama fasilitas (contoh: Perpustakaan)"
          required
          value={form.nama}
      ...
      <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4">
        {items.map((item) => (
          <div key={item.id} className="bg-white border border-ink/10 rounded-lg overflow-hidden">
            {item.foto_url && <img src={item.foto_url} alt={item.nama} className="w-full h-32 object-cover" />}
            <div className="p-3">
              <h3 className="font-display font-bold text-sm">{item.nama}</h3>
